    ...
    const sendOTP = async (email, id_transaksiKurirGuru, id_transaksiKurirSiswa) => {
        try {
            // Generate OTP
            const otp = otpGenerator.generate(4, { upperCaseAlphabets: false, specialChars: false, lowerCaseAlphabets: false });
    
            // Simpan OTP ke database atau sesuai kebutuhan Anda
            await otpService.create({ otp, email, id_transaksiKurirGuru, id_transaksiKurirSiswa });
    ...
    const verifyOTP = async (email, otp, id_transaksiKurir) => {
        try {
            ...
            if (user) {
                ...
                await otpService.find({ where: { email, id_transaksiKurir } });
